...
<div class="merchantinfo-form">
    ...
    <?= $form->field($model, 'principal_idcard_img')->widget(
        ...
        [
            'pluginOptions' => [
                'showUpload' => false,
                'initialPreview' => empty($model->principal_idcard_img)?'':[\yii\helpers\Url::to($model->principal_idcard_img)],
    ...
    <?= $form->field($model, 'principal_idcard_img2')->widget(FileInput::class,
        [
            'pluginOptions' => [
                'showUpload' => false,
                'initialPreview' => empty($model->principal_idcard_img2)?'':[\yii\helpers\Url::to($model->principal_idcard_img2)],
                'initialPreviewAsData' => true,
            ],
//            'pluginEvents' => [
...
//            ],
    ...
    <?= $form->field($model, 'business_license_img')->widget(FileInput::class,
        [
            'pluginOptions' => [
                'showUpload' => false,
                'initialPreview' => empty($model->business_license_img)?'':[\yii\helpers\Url::to($model->business_license_img)],
                'initialPreviewAsData' => true,
            ],
            'pluginEvents' => [
                "fileclear" => "function() { $('#products-image').val('');}",
...
<!--    --><?//= $form->field($model, 'updated_at')->textInput() ?>
    ...
    <div class="form-group">
        <?= Html::submitButton($model->isNewRecord ? '下一步' : 'Update', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>
